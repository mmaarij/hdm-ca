...
import * as PermissionDTOs from "../dtos/permission/request.dto";
import type {
...
export const grantPermission =
  ...
  > =>
    pipe(
      ...
      S.decodeUnknown(PermissionDTOs.GrantPermissionCommand)(input),
...
export const updatePermission =
  ...
  > =>
    pipe(
      // Transform raw input to branded command
      S.decodeUnknown(PermissionDTOs.UpdatePermissionCommand)(input),
      Effect.mapError((e) => new Error(`Invalid input: ${e}`)),
      Effect.flatMap((command) =>
        pipe(
          loadEntity(
...
export const revokePermission =
  ...
  > =>
    pipe(
      // Transform raw input to branded command
      S.decodeUnknown(PermissionDTOs.RevokePermissionCommand)(input),
      Effect.mapError((e) => new Error(`Invalid input: ${e}`)),
      Effect.flatMap((command) =>
        pipe(
          loadEntity(
...
export const listDocumentPermissions =
  (deps: PermissionWorkflowDeps) =>
  (
    input: ListDocumentPermissionsInput
  ): Effect.Effect<
    ListPermissionsResponse,
    NotFoundError | ForbiddenError | Error
  > =>
    pipe(
      ...
      S.decodeUnknown(PermissionDTOs.ListDocumentPermissionsQuery)(input),
      Effect.mapError((e) => new Error(`Invalid input: ${e}`)),
...
export const listUserPermissions =
  ...
  (
    ...
    pipe(
      // Transform raw input to branded query
      S.decodeUnknown(PermissionDTOs.ListUserPermissionsQuery)(input),
      Effect.mapError((e) => new Error(`Invalid input: ${e}`)),
      Effect.flatMap((query) =>
        pipe(
...
export const checkPermission =
  ...
  (
    ...
    pipe(
      // Transform raw input to branded query
      S.decodeUnknown(PermissionDTOs.CheckPermissionQuery)(input),
      Effect.mapError((e) => new Error(`Invalid input: ${e}`)),
      Effect.flatMap((query) =>
        pipe(
          Effect.all({
