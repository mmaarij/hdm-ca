...
import type {
  ...
  ValidateDownloadTokenInput,
  ValidateDownloadTokenQuery,
  DownloadFileInput,
  DownloadFileQuery,
  CleanupExpiredTokensInput,
  CleanupExpiredTokensCommand,
} from "../dtos/download-token/request.dto";
import * as DownloadTokenDTOs from "../dtos/download-token/request.dto";
import type {
...
export const generateDownloadLink =
  ...
  > =>
    pipe(
      S.decodeUnknown(DownloadTokenDTOs.GenerateDownloadLinkCommand)(input),
      Effect.mapError(
        (e) => new Error(`Invalid input for generateDownloadLink: ${e}`)
      ),
...
export const validateToken =
  (deps: DownloadTokenWorkflowDeps) =>
  (
    input: ValidateDownloadTokenInput
  ): Effect.Effect<ValidateTokenResponse, Error> =>
    pipe(
      S.decodeUnknown(DownloadTokenDTOs.ValidateDownloadTokenQuery)(input),
      Effect.mapError(
        (e) => new Error(`Invalid input for validateToken: ${e}`)
      ),
...
export const downloadFile =
  ...
  > =>
    pipe(
      S.decodeUnknown(DownloadTokenDTOs.DownloadFileQuery)(input),
      Effect.mapError((e) => new Error(`Invalid input for downloadFile: ${e}`)),
      Effect.flatMap((query) =>
        pipe(
          deps.tokenRepo.findByToken(query.token),
          Effect.flatMap(
...
export const cleanupExpiredTokens =
  ...
  (
    ...
    pipe(
      S.decodeUnknown(DownloadTokenDTOs.CleanupExpiredTokensCommand)(input),
      Effect.mapError(
        (e) => new Error(`Invalid input for cleanupExpiredTokens: ${e}`)
      ),
      Effect.flatMap((command) =>
        pipe(
          loadEntity(
